...
import type { GithubBranch, Project, Task, TaskPayload, TaskStatus, User, UserPayload } from "../../../types";
    ...
    <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}>
      <section className="modal" role="dialog" aria-modal="true" aria-label={title}>
        <header className="modal__header">
          <div><span className="eyebrow">OrbitTask</span><h2>{title}</h2></div>
...
function ProjectForm({ leads, busy, onClose, onSubmit }: { leads: User[]; busy: boolean; onClose: () => void; onSubmit: (payload: { name: string; description?: string; lead_developer_id: number }) => Promise<void> }) {
  const submit = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    const values = new FormData(event.currentTarget);
    return onSubmit({ name: String(values.get("name")), description: String(values.get("description")), lead_developer_id: Number(values.get("lead_developer_id")) });
  };
  return (
    <form className="form-grid" onSubmit={submit}>
      ...
      <label className="form-field form-field--wide">Lead developer (obligatoire)<select name="lead_developer_id" defaultValue="" required><option value="" disabled>Choisir un lead developer</option>{leads.map((lead) => <option value={lead.id} key={lead.id}>{lead.name}</option>)}</select></label>
      <footer className="modal__actions"><button type="button" className="secondary-button" onClick={onClose}>Annuler</button><button className="primary-button" disabled={busy || leads.length === 0}>{busy ? "Création…" : "Créer le projet"}</button></footer>
    </form>
  );
}
...
function Dashboard() {
  ...
  const isLeadDeveloper = currentUser.role === "lead_developer";
  const canManageTasks = isManager || isLeadDeveloper;
  ...
  const handleCreateBranch = (taskId: number) => run(async () => {
    const branch = await createGithubBranch(taskId);
    setBranches((prev) => ({ ...prev, [taskId]: branch }));
  ...
  }, [currentUser.id, isManager, isLeadDeveloper]);
  ...
  const assignableProjects = isLeadDeveloper ? projects.filter((project) => Number(project.lead_developer_id) === Number(currentUser.id)) : projects;
    ...
    return (
      <article className={`task-card task-card--${task.status}`} key={task.id}>
        <div className="task-card__top">
          <button className={`task-check ${task.status === "completed" ? "is-checked" : ""}`} aria-label={task.status === "completed" ? "Marquer active" : "Marquer terminée"} onClick={() => canManageTasks && void run(() => updateTask(task.id, { status: task.status === "completed" ? "pending" : "completed" }).then(() => undefined), "Statut mis à jour")} disabled={!canManageTasks}>
            {task.status === "completed" && <Icon name="check" />}
          </button>
          <span className="task-card__project"><Icon name="folder" />{task.project?.name ?? "Projet"}</span>
          {canManageTasks && <div className="card-actions"><button className="icon-button" aria-label="Modifier" onClick={() => setModal({ type: "task", item: task })}><Icon name="edit" /></button><button className="icon-button icon-button--danger" aria-label="Supprimer" onClick={() => window.confirm(`Supprimer la tâche « ${task.title} » ?`) && void run(() => deleteTask(task.id), "Tâche supprimée")}><Icon name="trash" /></button></div>}
        </div>
        <div className="task-card__content">
          <span className={`status-pill status-pill--${task.status}`}>{statusLabels[task.status]}</span>
          <h3>{task.title}</h3>
          <p>{task.description || "Aucune description ajoutée."}</p>
        </div>
        <footer className="task-card__footer">
          <div className="task-card__meta">
            <span className="task-card__date"><Icon name="calendar" /> {formatDay(task.start_date)}</span>
          ...
          {(mine && ["assigned", "in_progress"].includes(task.status) || action) && (
            <div className="task-card__actions">
              {mine && ["assigned", "in_progress"].includes(task.status) && (branches[task.id]
                ? <a className="task-action task-action--branch" href={branches[task.id].github_url ?? "#"} target="_blank" rel="noreferrer"><Icon name="folder" />{branches[task.id].branch_name}</a>
                : <button className="task-action task-action--branch" onClick={() => void handleCreateBranch(task.id)}><Icon name="folder" />Créer une branche</button>)}
              {action && <button className={`task-action task-action--${action}`} onClick={() => void run(() => taskAction(task.id, action!), `Tâche : ${actionLabel.toLowerCase()}`)}>{actionLabel}<Icon name="chevron" /></button>}
            </div>
          )}
        </footer>
      </article>
  ...
  return (
    <div className="dashboard-shell">
        ...
        <div className="workspace">
          ...
          {section === "overview" && isManager && <>
            <section className="hero-banner">
              <div className="hero-banner__shape hero-banner__shape--a" aria-hidden="true" />
              <div className="hero-banner__shape hero-banner__shape--b" aria-hidden="true" />
              <div className="hero-banner__text">
                <span className="eyebrow eyebrow--light">TABLEAU DE BORD</span>
                <h1>Bonjour, {currentUser.name?.split(" ")[0]} 👋</h1>
                <p>Voici où en est votre équipe aujourd’hui.</p>
              </div>
              <button className="primary-button hero-banner__cta" onClick={() => setModal({ type: "task" })}><Icon name="plus" />Nouvelle tâche</button>
            </section>
            <section className="stats-grid">
              <article className="stats-grid__card stats-grid__card--blue"><span className="stat-icon stat-icon--blue"><Icon name="tasks" /></span><div><strong>{tasks.length}</strong><span>Tâches au total</span></div></article>
              <article className="stats-grid__card stats-grid__card--violet"><span className="stat-icon stat-icon--violet"><Icon name="clock" /></span><div><strong>{inProgress}</strong><span>En cours</span></div></article>
              <article className="stats-grid__card stats-grid__card--mint"><span className="stat-icon stat-icon--mint"><Icon name="check" /></span><div><strong>{completed}</strong><span>Terminées</span></div></article>
              <article className="stats-grid__card stats-grid__card--sand"><span className="stat-icon stat-icon--sand"><Icon name="users" /></span><div><strong>{developers.filter((user) => user.is_active !== false).length}</strong><span>Membres actifs</span></div></article>
            </section>
          ...
          {section === "tasks" && <>
            <header className="page-heading"><div><span className="eyebrow">PLANIFICATION</span><h1>{canManageTasks ? "Gestion des tâches" : "Mes tâches"}</h1><p>{canManageTasks ? "Planifiez le travail et suivez l’avancement de l’équipe." : "Retrouvez les tâches disponibles et celles qui vous sont assignées."}</p></div>{canManageTasks && <button className="primary-button" onClick={() => setModal({ type: "task" })} disabled={assignableProjects.length === 0}><Icon name="plus" />Nouvelle tâche</button>}</header>
            ...
            {isLeadDeveloper && assignableProjects.length === 0 && !loading && <div className="alert alert--info">Aucun projet ne vous a été confié comme lead developer pour le moment.</div>}
          ...
          {section === "projects" && isManager && <>
            <header className="page-heading"><div><span className="eyebrow">ORGANISATION</span><h1>Projets</h1><p>Regroupez les tâches et désignez un lead pour chaque projet.</p></div><button className="primary-button" onClick={() => setModal({ type: "project" })} disabled={leads.length === 0}><Icon name="plus" />Nouveau projet</button></header>
            {leads.length === 0 && !loading && <div className="alert alert--info">Créez d’abord un compte Lead developer pour pouvoir créer un projet.</div>}
      ...
      {modal?.type === "task" && <Modal title={modal.item ? "Modifier la tâche" : "Créer une nouvelle tâche"} onClose={() => setModal(null)}><TaskForm task={modal.item} projects={assignableProjects} users={developers} busy={busy} onClose={() => setModal(null)} onSubmit={(payload) => run(() => modal.item ? updateTask(modal.item.id, payload).then(() => undefined) : createTask(payload).then(() => undefined), modal.item ? "Tâche mise à jour" : "Tâche créée")} /></Modal>}
